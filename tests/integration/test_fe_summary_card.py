...
def test_optimized_tab_strip_renders_when_optimized_present(
    page: Page, base_url: str, reseed
) -> None:
    """A real plan in safe mode produces result.optimized, which renders the
    Minimum Wait / Optimized tab strip."""
    import re as _re

    reseed("empty")
    plan_origin_to_dest(page, base_url)
    expect(page.get_by_role("button", name="Minimum Wait")).to_be_visible()
    # The optimized button label includes "Optimized (+...)" — match by prefix.
    expect(page.get_by_role("button", name=_re.compile(r"^Optimized"))).to_be_visible()


def test_optimized_tab_strip_hidden_when_optimized_missing(
    page: Page, base_url: str, reseed
) -> None:
    """When the backend omits `optimized`, the tab strip isn't rendered."""
    reseed("empty")
    mock_route_response(page, result=two_hop_result())  # no `optimized` key
    plan_origin_to_dest(page, base_url)
